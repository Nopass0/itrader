...
export default function GateAccountDetailPage() {
  ...
  const loadAccountBalance = useCallback(async () => {
    ...
    try {
      const response = await apiClient.get(`/gate/accounts/${accountId}/balance`);
  ...
  useEffect(() => {
    ...
    const loadAccountData = async () => {
      ...
      try {
        // Load account info
        const accountResponse = await apiClient.get(`/gate/accounts`);
    ...
    loadAccountData();
  }, [accountId, loadAccountBalance, loadAccountStats, loadTabData]);

...
1. **Серверная настройка** - для настройки окружения на сервере, где уже установлены Node.js, npm и Bun:
   ```bash
   ./scripts/server-setup.sh
   ```
   Этот скрипт фокусируется только на установке PostgreSQL, настройке базы данных и установке зависимостей проекта.
...
5. **Модификация скриптов настройки** - при добавлении новых зависимостей или переменных окружения обновлять скрипты `./scripts/setup.sh`, `./scripts/quicksetup.sh` и `./scripts/server-setup.sh`

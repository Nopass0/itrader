...
1. **Настройка базы данных** - для установки и настройки PostgreSQL и зависимостей проекта:
   ```bash
   ./scripts/db-setup.sh
   ```
   Этот скрипт устанавливает PostgreSQL напрямую с официального сайта (без использования репозиториев Ubuntu), настраивает базу данных и устанавливает зависимости проекта.

  ...
  static async startGmailOAuth(
    ...
    try {
      ...
      try {
        ...
        credentials = credentialsContent.web || credentialsContent.installed || credentialsContent;
        
        if (!credentials.client_id || !credentials.client_secret) {
          throw new Error('Invalid credentials file');
        }
      } catch (error) {
        handleError(new Error('Gmail credentials not configured. Please set up gmail-credentials.json'), callback);
        return;
      }
      ...
      const redirectUri = credentials.redirect_uris?.[0] || `${process.env.FRONTEND_URL || 'http://localhost:3000'}/panel/gmail-callback`;
  ...
  static async completeGmailOAuth(
    ...
    try {
      ...
      if (stateAge > 15 * 60 * 1000) {
        throw new Error('Время авторизации истекло. Пожалуйста, начните процесс заново.');
      }
      ...
      try {
        ...
        if (error.message?.includes('invalid_grant')) {
          throw new Error('Код авторизации недействителен или уже был использован. Пожалуйста, начните процесс авторизации заново.');
        } else if (error.message?.includes('redirect_uri_mismatch')) {
          throw new Error('Неверный redirect URI. Проверьте настройки OAuth в Google Console.');
        } else {
          throw new Error(`Ошибка при обмене кода на токен: ${error.message || 'Неизвестная ошибка'}`);
        }
      }
      
      if (!tokens.refresh_token) {
        throw new Error('Не получен refresh token. Возможно, приложение уже авторизовано. Попробуйте отозвать доступ в настройках Google и повторить авторизацию.');
      }
